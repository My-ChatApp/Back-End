# Adds date_of_birth + gender to app.users on an existing Postgres volume.
# Docker init scripts in init/ only run when the data volume is first created.

$ErrorActionPreference = "Stop"
$scriptDir = Split-Path -Parent $MyInvocation.MyCommand.Path
$sqlFile = Join-Path $scriptDir "init\03-profile-fields.sql"
...
Write-Host "Migration applied: 03-profile-fields.sql"
