...
export const env = {
  ...
  mediaPresignExpirationMinutes: readInt('MEDIA_PRESIGN_EXPIRATION_MINUTES', 15),
  /** HTTP sidecar (Docker service `magika`, port 8090). */
  magikaServiceUrl: process.env.MAGIKA_SERVICE_URL?.trim() || 'http://localhost:8090',
  ...
  magikaStartupRetryMaxAttempts: readInt('MAGIKA_STARTUP_RETRY_MAX_ATTEMPTS', 10),
  ...
  magikaClientRateLimitWindowMs: readInt('MAGIKA_CLIENT_RATE_LIMIT_WINDOW_MS', 60_000),
  ...
  mediaMagikaSkipOnMagicMatch: readBool('MEDIA_MAGIKA_SKIP_ON_MAGIC_MATCH', true),
} as const;
